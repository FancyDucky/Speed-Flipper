...
async function openOverlayPopout() {
  ...
  if (canDocPip) {
    ...
    // If the user expected PiP but we can't even see the API, this is usually:
    // - a browser/version mismatch, OR
    // - the feature is disabled by policy on this origin, OR
    // - a cross-origin embed (not the case here), OR
    // - a privacy/extension setting.
    console.warn(
      "Document Picture-in-Picture API not available on this page; falling back to a normal popup."
    );
    alert(
      "Document Picture-in-Picture is not available on this page, so the overlay will open as a normal popup.\n\n" +
        "A normal popup cannot be forced to stay always on top.\n\n" +
        "If you expected PiP here, open DevTools → Console and look for the warning about Document PiP."
    );
